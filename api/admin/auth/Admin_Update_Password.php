...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   ...
   // Check if user with username exists in database
    ...
    $user=$api_users_table_class_call::getAdminUserByKey($user_pubkey);
    if (count($user) == 0) {
        $text = $api_response_class_call::$invalidUserDetail;
        $errorcode = $api_error_code_class_call::$internalUserWarning;
        $maindata = [];
        $hint = ["Ensure data sent is valid and user data is in database."," Admin User not found"];
        $linktosolve = "https://";
        $api_status_code_class_call->respondBadRequest($maindata,$text,$hint,$linktosolve,$errorcode);
    }
    ...
    $updateSuccess = $api_users_table_class_call::updateAdminPassword($user_pubkey, $hashPassword);
